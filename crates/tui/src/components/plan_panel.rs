...
pub fn draw(frame: &mut Frame, area: Rect, state: &PlanPanelState, theme: &Theme) {
    ...
    if total > 0 {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(format!(" {done}/{total}"), ty::secondary(theme)),
        ]));
        lines.push(Line::from(""));
        ...
        for todo in &plan.frontmatter.todos {
            let (marker, marker_color) = match todo.status {
                ...
                TodoStatus::InProgress => ("\u{25B8}", theme.warning),
            ...
            let content_style = match todo.status {
                TodoStatus::Completed => ty::disabled(theme),
                TodoStatus::InProgress => ty::heading(theme),
                TodoStatus::Cancelled => ty::muted(theme).add_modifier(Modifier::ITALIC),
                TodoStatus::Pending => ty::secondary(theme),
            };
    ...
    if !plan.body.is_empty() {
        ...
        let max_w = inner.width.saturating_sub(PAD_H * 2) as usize;
